...
const CompanyKeywordPage = () => {
  ...
  const [newsData, setNewsData] = useState({}); // 키워드에 대한 뉴스 데이터를 저장
  ...
  useEffect(() => {
    const fetchHotKeywords = async () => {
      ...
      setError(null);
      try {
        const response = await axiosInstance.get(`/api/companies/hotkeyword_with_news`, {
          params: { corp_name: company.company_name },
        });
        setKeywords(Object.keys(response.data.keywords)); // 키워드를 상태에 저장
        setNewsData(response.data.keywords); // 키워드에 대한 뉴스 데이터를 저장
      } catch (error) {
      ...
      <div className="p-8 flex flex-col items-center justify-center min-h-screen pt-20">
        ...
        >
          <ModalContent>
            ...
            <ModalBody>
              {newsData[selectedKeyword]?.length > 0 ? ( // 해당 키워드에 대한 뉴스가 있을 때 리스트 표시
                  <ul>
                    {newsData[selectedKeyword].map((news, index) => (
                        <li
                            key={index}
